...
## On-site forms backend — env (brief Update 8)
The forms are built and work in dev with no config (accept + log, skip the spam challenge). For
production, set the env vars in `.env.example` and create the table:
- [ ] **Own Postgres** — create `form_submissions` from `docs/forms-schema.sql` in your Supabase/Postgres project; set `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` (and optionally `SUBMISSIONS_TABLE`, `FORMS_ENCRYPTION_KEY`).
- [ ] **Spam** — `NEXT_PUBLIC_TURNSTILE_SITE_KEY` + `TURNSTILE_SECRET_KEY` (Cloudflare Turnstile).
- [ ] **Email** — `RESEND_API_KEY` + `FORMS_FROM_EMAIL` + `FORMS_TEAM_EMAIL` (verified sending domain).
- [ ] **Misc** — `FORMS_IP_SALT`, and `ADMIN_EXPORT_TOKEN` to unlock `/api/admin/export` for the quarterly review.
- [ ] **Privacy contact** — set `CONTACT_EMAIL` in `lib/config.ts` so `/privacy` shows a real deletion-request address.
