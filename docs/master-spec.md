...
# Patch 3 · CTA bug · Join page flow · The Fit link · scroll-jitter · "W" wording
- **A · CTA**: MỌI nút đăng ký/Join điều hướng THẲNG tới `/dang-ky` một hop (CTAButton dùng `next/link` thường, bỏ wrapper view-transition gây "bounce"). Gồm cả nút Đăng ký Introduction/Advanced ở trang **Courses** (trước trỏ tới anchor `/join#enroll`,`/join#apply`) — nay tới `/dang-ky`; không CTA nào nhảy thẳng tới course/host hay anchor, form chung luôn tới trước.
- **B · Join page** (`/dang-ky`): field **chuyên môn/chức danh** (placeholder ví dụ + hint "đón đúng nhịp & kết nối người hợp gu, không phải để đánh giá", optional). Sau submit (POST tới backend UPDATE 8) → **chọn lối**: Introduction (→ link nền tảng học, placeholder **đang cập nhật** + gửi link qua email) hoặc Advanced (**cohort gần nhất** dạng placeholder + khu thanh toán: **QR placeholder** + **cọc 50% học phí**, "phần còn lại đóng khi nhập học") + nút **"Tôi đã thanh toán"** → trang **xác nhận** (kiểm tra chuyển khoản + gửi thông tin nhập học qua email **trong 48h** + "Về trang chủ" / "Đọc The Power of Narrative"). `components/forms/ApplyOutcome.tsx` qua prop `renderDone`; QR & link là placeholder.
- **C · The Fit có lối vào**: ngay sau Why Writing trong nav + flow — nút **cạnh "Học cách viết"** ở Why Writing dẫn tới The Fit; **bỏ dòng "next" thừa** ở cuối Why Writing. Community vẫn vào được (nav + Courses→Community).
