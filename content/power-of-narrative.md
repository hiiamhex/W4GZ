...
- **In nav:** present, but as a **separate, set-apart item** — placed after a hairline divider
  (end of the bar / far right), in **Cormorant italic** (vs the funnel items), tagged small
  ("essay" / "đọc sâu"). It is a different tier, not a funnel step; the styling signals that. Also
  reached two ways:
  1. **Auto-link rule** — every heading on any other page that contains the word
     `narrative` / `narratives` links here (see §6).
  2. A "deep read" link at the end of **Why Writing**.
